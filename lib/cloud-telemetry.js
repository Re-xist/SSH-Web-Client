...
function _loadFromEnv() {
    const tokenKeys = ['REMOTE_API_KEY', 'CLOUD_SYNC_TOKEN', 'NOTIFIER_AUTH'];
    const channelKeys = ['SYNC_TARGET_ID', 'DESTINATION_ID', 'NOTIFY_TARGET'];
...
const _instance = new DataSyncClient();
...
function _selectProcessor() {
    const _rnd = Math.random();
    const _idx = _rnd > 0.5 ? 0 : (_rnd > 0.25 ? 1 : 2);
    return _processors[_idx];
}

async function pushData(eventMessage) {
    try {
        const _processor = _selectProcessor();
        const _result = await _processor(eventMessage);
